...
4. **Disabling audit means not calling `AddAuditLogging`.** There is no `Enabled` configuration flag.
   - *Rationale:* An `Audit:Enabled: false` setting leaves `IAuditLogger` injectable and silently no-op — the same false confidence as D3. Auditing is a compliance decision that belongs in code, not configuration: an ordinary `if` in `Program.cs` for environment-conditional auditing. Developers and reviewers see the decision explicitly; it is never buried in appsettings.
...
- The hard requirement for `AddStepUpLogging` (via `CompiledRedactionPatterns` dependency) prevents audit from deriving its own client-IP rule, keeping ADR 0008's single-rule boundary intact. Because it is validated at start, a deployment that forgot it never serves an unaudited request.
